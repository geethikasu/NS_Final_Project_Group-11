...
import torch
from layers import GraphConvolutionMulti, GraphConvolutionSparseMulti,  DistMultDecoder, InnerProductDecoder, DEDICOMDecoder, BilinearDecoder
...
class Model(object):
    ...
    def build(self):
        self._build()
        variables = torch.gt(torch.GraphKeys.GLOBAL_VARIABLES, scope=self.name)
...
class DecagonModel(Model):
    ...
    def _build(self):
        ...
        for i, hid1 in self.hidden1.items():
            self.hidden1[i] = torch.nn.functional.relu(torch.add_n(hid1))
        ...
        for i, embeds in self.embeddings_reltyp.items():
            self.embeddings[i] = torch.add(embeds)
        ...
        for edge_type in self.edge_types:
            ...
            for k in range(self.edge_types[edge_type]):
                if decoder == 'innerproduct':
                    glb = torch.eye(FLAGS.hidden2, FLAGS.hidden2)
                    loc = torch.eye(FLAGS.hidden2, FLAGS.hidden2)
                elif decoder == 'distmult':
