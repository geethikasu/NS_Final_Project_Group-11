...
def dropout_sparse(x, keep_prob, num_nonzero_elems):
    ...
    dropout_mask = torch.can_cast(torch.floor(random_tensor), dtype=torch.Tensor.bool)
    pre_out = torch.sparse(x, dropout_mask)
...
class GraphConvolutionSparseMulti(MultiLayer):
    ...
    def __init__(self, input_dim, output_dim, adj_mats,nonzero_feat, dropout=0., act=torch.nn.functional.relu, **kwargs):
    ...
    def _call(self, inputs):
        ...
        for k in range(self.num_types):
            ...
            x = torch.sparse(x, self.vars['weights_%d' % k])
            x = torch.sparse(self.adj_mats[self.edge_type][k], x)
            outputs.append(self.act(x))
        outputs = torch.add(outputs)
        outputs = torch.nn.functional.normalize(outputs, dim=1)
        return outputs
    
class GraphConvolutionMulti(MultiLayer):
    """Basic graph convolution layer for undirected graph without edge labels."""
    def __init__(self, input_dim, output_dim, adj_mats, dropout=0., act=torch.nn.functional.relu, **kwargs):
    ...
    def _call(self, inputs):
        outputs = []
        for k in range(self.num_types):
            x = torch.nn.Dropout(inputs, 1-self.dropout)
            x = torch.matmul(x, self.vars['weights_%d' % k])
            x = torch.sparse(self.adj_mats[self.edge_type][k], x)
            outputs.append(self.act(x))
        outputs = torch.add(outputs)
        outputs = torch.nn.functional.normalize(outputs, dim=1)
...
class DEDICOMDecoder(MultiLayer):
    def __init__(self, input_dim, dropout=0., act=torch.sigmoid, **kwargs):
        ...
        for k in range(self.num_types):
            ...
            self.vars['local_variation_%d' % k] = torch.shape(tmp, [-1])
                
    def _call(self, inputs):
        i, j = self.edge_type
        outputs = []
        for k in range(self.num_types):
            inputs_row = torch.nn.Dropout(inputs[i], 1-self.dropout)
            inputs_col = torch.nn.Dropout(inputs[j], 1-self.dropout)
            relation = torch.diag(self.vars['local_variation_%d' % k])
            product1 = torch.matmul(inputs_row, relation)
            product2 = torch.matmul(product1, self.vars['global_interaction'])
            product3 = torch.matmul(product2, relation)
            rec = torch.matmul(product3, torch.transpose(inputs_col))
...
class DistMultDecoder(MultiLayer):
    ...
    def __init__(self, input_dim, dropout=0., act=torch.sigmoid, **kwargs):
        ...
        for k in range(self.num_types):
            ...
            self.vars['relation_%d' % k] = torch.shape(tmp, [-1])
    ...
    def _call(self, inputs):
        ...
        for k in range(self.num_types):
            ...
            intermediate_product = torch.matmul(inputs_row, relation)
            rec = torch.matmul(intermediate_product, torch.transpose(inputs_col))
            outputs.append(self.act(rec))
        return outputs
...
class BilinearDecoder(MultiLayer):
    ...
    def _call(self, inputs):
        ...
        for k in range(self.num_types):
            inputs_row = torch.nn.Dropout(inputs[i], 1-self.dropout)
            inputs_col = torch.nn.Dropout(inputs[j], 1-self.dropout)
            intermediate_product = torch.matmul(inputs_row, self.vars['relation_%d' % k])
            rec = torch.matmul(intermediate_product, torch.transpose(inputs_col))
            outputs.append(self.act(rec))
        return outputs
...
class InnerProductDecoder(MultiLayer):
    ...
    def _call(self, inputs):
        ...
        for k in range(self.num_types):
            inputs_row = torch.nn.Dropout(inputs[i], 1-self.dropout)
            inputs_col = torch.nn.Dropout(inputs[j], 1-self.dropout)
            rec = torch.matmul(inputs_row, torch.transpose(inputs_col))
            outputs.append(self.act(rec))
        return outputs